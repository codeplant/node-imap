var tls = require("tls"),
  ...
  utf7 = require("@codeplant/utf7").imap;
...
Connection.prototype.connect = function () {
  ...
  if (config.tls) {
    ...
    for (var k in config.tlsOptions) tlsOptions[k] = config.tlsOptions[k];
    tlsOptions.socket = socket;
  }

  if (config.tls) this._sock = tls.connect(tlsOptions, onconnect);
  else {
  ...
  parser.on("body", function (stream, info) {
    var msg = self._curReq.fetchCache[info.seqno],
      toget;
  ...
  parser.on("continue", function (info) {
    ...
    if (type === "IDLE") {
      ...
      self._curReq.oauthError = Buffer.from(info.text, "base64").toString(
        "utf8"
      );
...
Connection.prototype.append = function (data, options, cb) {
  ...
  if (literal) this._queue[this._queue.length - 1].literalAppendData = data;
  else this._queue[this._queue.length - 1].appendData = data;
};
  ...
  for (var i = 0, len = items.length; i < len; ++i) {
    if (isFlags) {
      ...
      if (RE_INVALID_KW_CHARS.test(items[i])) {
        throw new Error(
          'The keyword "' + items[i] + '" contains invalid characters'
        );
      }
    }
  }
...
Connection.prototype._move = function (which, uids, boxTo, cb) {
  ...
  if (this.serverSupports("MOVE")) {
    ...
    this._box.permFlags.indexOf("\\Deleted") === -1 &&
    this._box.flags.indexOf("\\Deleted") === -1
  ) {
    throw new Error(
      "Cannot move message: " + "server does not allow deletion of messages"
    );
  } else {
    var deletedUIDs,
      task = 0,
...
Connection.prototype._resUntagged = function (info) {
  ...
  if (type === "bye") this._sock.end();
    ...
    if (this._box) this._box.messages.new = info.num;
  } else if (type === "flags") {
    if (!this._box && RE_OPENBOX.test(this._curReq.type))
      this._createCurrentBox();
...
Connection.prototype._resTagged = function (info) {
  ...
  if (req.bodyEmitter) {
    ...
    if (err) bodyEmitter.emit("error", err);
    process.nextTick(function () {
...
Connection.prototype._enqueue = function (fullcmd, promote, cb) {
  ...
  if (promote) this._queue.unshift(info);
  else this._queue.push(info);
  ...
  for (var i = 0, len = uids.length, intval; i < len; ++i) {
    if (typeof uids[i] === "string") {
      if (uids[i] === "*" || uids[i] === "*:*") {
        if (len > 1) uids = ["*"];
        break;
      } else if (RE_NUM_RANGE.test(uids[i])) continue;
    }
    intval = parseInt("" + uids[i], 10);
    if (isNaN(intval)) {
      var err = new Error(
        'UID/seqno must be an integer, "*", or a range: ' + uids[i]
      );
...
function buildSearchQuery(options, extensions, info, isOrChild) {
  ...
  for (var i = 0, len = options.length; i < len; ++i) {
    var criteria = isOrChild ? options : options[i],
      args = null,
      modifier = isOrChild ? "" : " ";
    if (typeof criteria === "string") criteria = criteria.toUpperCase();
    else if (Array.isArray(criteria)) {
      if (criteria.length > 1) args = criteria.slice(1);
      if (criteria.length > 0) criteria = criteria[0].toUpperCase();
    } else
      throw new Error(
      ...
      switch (criteria) {
        ...
        case "TO":
          if (!args || args.length !== 1)
            throw new Error(
              "Incorrect number of arguments for search option: " + criteria
            );
          val = buildString(args[0]);
          if (info && val[0] === "{") info.hasUTF8 = true;
        ...
        case "HEADER":
          ...
          if (info && val[0] === "{") info.hasUTF8 = true;
          searchargs +=
            modifier + criteria + ' "' + escape("" + args[0]) + '" ' + val;
          break;
        case "UID":
          if (!args)
            throw new Error(
          ...
          if (args.length === 0) throw new Error("Empty uid list");
        ...
        case "X-GM-THRID": // Gmail thread ID
          ...
          if (!args || args.length !== 1)
            ...
            val = "" + args[0];
            if (!RE_INTEGER.test(args[0])) throw new Error("Invalid value");
          }
          searchargs += modifier + criteria + " " + val;
          break;
        ...
        default:
          ...
          } else throw new Error("Unexpected search option: " + criteria);
      }
    }
    if (isOrChild) break;
  }
