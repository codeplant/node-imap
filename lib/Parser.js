var EventEmitter = require("events").EventEmitter,
  ReadableStream =
    require("stream").Readable || require("readable-stream").Readable,
  inherits = require("util").inherits,
  inspect = require("util").inspect;

var utf7 = require("@codeplant/utf7").imap,
...
function Parser(stream, debug) {
  if (!(this instanceof Parser)) return new Parser(stream, debug);
  ...
  this._cbReadable = function () {
    if (self._ignoreReadable) return;
    if (self._literallen > 0 && !self._body) self._tryread(self._literallen);
    else self._tryread();
  };
...
Parser.prototype.setStream = function (stream) {
  if (this._stream) this._stream.removeListener("readable", this._cbReadable);
...
Parser.prototype._parse = function (data) {
  ...
  if (this._literallen > 0) {
    if (this._body) {
      ...
      if (datalen >= this._literallen) {
        ...
        if (datalen > litlen) body.push(data.slice(0, litlen));
        else body.push(data);
        body.push(null);
      } else {
  ...
  while (i < datalen) {
    ...
    if (idxlf === -1) {
      ...
      if (RE_PRECEDING.test(this._buffer)) {
        ...
        if (firstChar === "*") this._resUntagged();
        else if (firstChar === "A") this._resTagged();
        else if (firstChar === "+") this._resContinue();
  ...
  if (this._literallen === 0 || this._body) this._tryread();
};
...
Parser.prototype._resUntagged = function () {
  ...
  if ((m = RE_BODYLITERAL.exec(this._buffer))) {
    ...
    if (m[4] !== undefined) textCode = parseTextCode(m[4], this._literals);
    ...
    if (
      ...
      if (m[5]) val = parseExpr(m[5], this._literals);
      else val = [];
    } else if (type === "list" || type === "lsub" || type === "xlist")
      val = parseBoxList(m[5], this._literals);
    else if (type === "id") val = parseId(m[5], this._literals);
    else if (type === "status") val = parseStatus(m[5], this._literals);
    else if (type === "fetch")
      val = parseFetch.call(this, m[5], this._literals, num);
    else if (type === "namespace") val = parseNamespaces(m[5], this._literals);
    else if (type === "esearch") val = parseESearch(m[5], this._literals);
    else if (type === "quota") val = parseQuota(m[5], this._literals);
    else if (type === "quotaroot") val = parseQuotaRoot(m[5], this._literals);
    else val = m[5];
...
Parser.prototype._resContinue = function () {
  ...
  if (m[1] !== undefined) textCode = parseTextCode(m[1], this._literals);
...
function parseTextCode(text, literals) {
  ...
  else return { key: r[0], val: r.length === 2 ? r[1] : r.slice(1) };
}
...
function parseESearch(text, literals) {
  ...
  for (var i = 1, len = r.length, key, val; i < len; i += 2) {
    key = r[i].toLowerCase();
    val = r[i + 1];
    if (key === "all") val = val.toString().split(",");
    attrs[key] = val;
  }
  ...
  for (n = 0; n < 3; ++n) {
    if (r[n]) {
      ...
      for (i = 0, len = r[n].length; i < len; ++i) {
        ...
        if (ns.length > 2) nsobj.extensions = {};
...
function parseFetch(text, literals, seqno) {
  var list = parseExpr(text, literals)[0],
    attrs = {},
    m,
    body;
  ...
  for (var i = 0, len = list.length, key, val; i < len; i += 2) {
    ...
    val = list[i + 1];
    if (key === "envelope") val = parseFetchEnvelope(val);
    else if (key === "internaldate") val = new Date(val);
    else if (key === "modseq")
...
function parseBodyStructure(cur, literals, prefix, partID) {
  ...
  if (prefix === undefined) {
    ...
    if (result.length) ret = parseBodyStructure(result, literals, "", 1);
  } else {
    var part,
      partLen = cur.length,
      next;
    if (Array.isArray(cur[0])) {
      ...
      if (
        ...
        part.lines = cur[next++];
      if (typeof cur[1] === "string" && partLen > next) part.md5 = cur[next++];
    }
...
function parseStructExtra(part, partLen, cur, next) {
  if (partLen > next) {
    ...
    } else if (cur[next] !== null) disposition.type = cur[next];

    if (disposition.type === null) part.disposition = null;
    else part.disposition = disposition;
  ...
  if (partLen > next) {
    ...
    else part.language = null;
    ++next;
  }
  if (partLen > next) part.location = cur[next++];
  if (partLen > next) {
  ...
  if (Array.isArray(list)) {
    ...
    for (var i = 0, len = list.length, addr; i < len; ++i) {
      ...
      if (addr[2] === null) {
        ...
        if (inGroup) curGroup.addresses.push(info);
        else if (!inGroup) addresses.push(info);
      }
      list[i] = addr;
    }
    if (inGroup) {
...
function parseExpr(o, literals, result, start, useBrackets) {
  ...
  if (useBrackets === undefined) useBrackets = true;
  if (!result) result = [];
  if (typeof o === "string") {
    o = { str: o };
    isTop = true;
  ...
  for (var i = start, len = o.str.length; i < len; ++i) {
    ...
    } else if (o.str[i] === "\\") escaping = !escaping;
    else if (o.str[i] === '"') {
      if (!escaping) inQuote = false;
      escaping = false;
    }
...
function convStr(str, literals) {
  if (str[0] === '"') {
    ...
    if (Buffer.isBuffer(l)) l = l.toString("utf8");
    return l;
  }
...
function repeat(chr, len) {
  var s = "";
  for (var i = 0; i < len; ++i) s += chr;
  return s;
}

function decodeBytes(buf, encoding, offset, mlen, pendoffset, state, nextBuf) {
  if (!jsencoding) jsencoding = require("../deps/encoding/encoding");
  if (jsencoding.encodingExists(encoding)) {
    ...
    if (state.remainder !== undefined) {
      ...
      try {
        ...
        if (e.message.indexOf("Seeking") === 0) isPartial = true;
      }
    }
    if (!isPartial && nextBuf) {
      ...
      var lookahead,
        lookaheadBuf = Buffer.allocUnsafe(buf.length + nextBuf.length);
    ...
    if (ret !== undefined) {
      ...
      state.buffer = buf;
      if (!state.curReplace) state.replaces.push((state.curReplace = []));
...
function qEncReplacer(match, byte) {
  if (match === "_") return " ";
  else return String.fromCharCode(parseInt(byte, 16));
}
...
function parseHeader(str, noDecode) {
  ...
  for (i = 0; i < len; ++i) {
    if (lines[i].length === 0) break; // empty line separates message's header and body
    if (lines[i][0] === "\t" || lines[i][0] === " ") {
      if (!Array.isArray(header[h])) continue; // ignore invalid first line
      // folded header content
      val = lines[i];
      if (!noDecode) {
        if (RE_ENCWORD_END.test(lines[i - 1]) && RE_ENCWORD_BEGIN.test(val)) {
      ...
      if (m) {
        ...
        if (m[2]) {
          if (header[h] === undefined) header[h] = [m[2]];
          else header[h].push(m[2]);
        } else header[h] = [""];
      } else break;
    }
  }
